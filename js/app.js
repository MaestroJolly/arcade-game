// variables for score, levels, score and level DOM selection

let score = 0;
let level = 1;
// let mute = document.querySelector(".mute");
// let unMute = document.querySelector(".unmute");
let levelNumber = document.querySelector(".level-number");
let scoreDisplay = document.querySelector(".score");
let muteSound = document.querySelector(".fa-volume-up");
let unMuteSound = document.querySelector(".fa-volume-off");

let levelName = levelNumber.textContent;
let theLevelName = `${levelName} ${level}`;
levelNumber.innerHTML = theLevelName;

let scoreLabel = scoreDisplay.textContent;
let theScore = `${scoreLabel} ${score}`;
scoreDisplay.innerHTML = theScore;


// construction function for game sound

const Sound = function (src) {
    this.sound = document.createElement("audio");
    this.sound.src = src;
    this.sound.setAttribute("preload", "auto");
    this.sound.setAttribute("controls", "none");
    this.sound.style.display = "none";
    document.body.appendChild(this.sound);
    this.play = function () {
        this.sound.play();
    }
    this.stop = function () {
        this.sound.pause();
    }
}




// Enemies our player must avoid
var Enemy = function(posX, posY) {
    // Variables applied to each of our instances go here,
    // we've provided one for you to get started

    // The image/sprite for our enemies, this uses
    // a helper we've provided to easily load images
    this.x = posX;
    this.y = posY;
    this.sprite = 'images/enemy-bug.png';
};

// Update the enemy's position, required method for game
// Parameter: dt, a time delta between ticks
Enemy.prototype.update = function(dt) {
    // You should multiply any movement by the dt parameter
    // which will ensure the game runs at the same speed for
    // all computers.
    if (this.x < 505) {
        this.x += (level * 50 * dt);
        gameSound.play();
    } else {
        this.x = -90;
    }
    // conditional statement that handles collision
    if (this.x < player.x + 40 && this.x + 60 > player.x && this.y < player.y + 60 && this.y + 40 > player.y) {
        if (score === 0){
            zeroScore();
        }else{
            removeOne();
        }
    }else if (score === 5) {
        level += 1;
        score = 0;
        theScore = `${scoreLabel} ${score}`;
        scoreDisplay.innerHTML = theScore;
        theLevelName = `${levelName} ${level}`;
        levelNumber.innerHTML = theLevelName;
        collisionSound.play();
    }
};

// Draw the enemy on the screen, required method for game
Enemy.prototype.render = function() {
    ctx.drawImage(Resources.get(this.sprite), this.x, this.y);
};

// Player to avoid enemies
var Player = function (posX, posY) {
    // Variables applied to each of our instances go here,
    // we've provided one for you to get started

    // The image/sprite for our player, this uses
    // a helper we've provided to easily load images
    this.x = posX;
    this.y = posY;
    this.sprite = 'images/char-princess-girl.png';
};

// Update the player's position, required method for game
// Parameter: dt, a time delta between ticks
Player.prototype.update = function(dt) {
    // You should multiply any movement by the dt parameter
    // which will ensure the game runs at the same speed for
    // all computers.
    	if (player.y < -30) {
            addOne();
        }
};

// Draws the player on the screen, required method for game
Player.prototype.render = function() {
    ctx.drawImage(Resources.get(this.sprite), this.x, this.y);
};

Player.prototype.handleInput = function (navInput){
        if (navInput == 'left' && this.x > 0) {
            this.x -= 100;
        }
        if (navInput == 'right' && this.x < 400) {
            this.x += 100;
        }
        if (navInput == 'up' && this.y > -30) {
            this.y -= 90;
        }
        if (navInput == 'down' && this.y < 400) {
            this.y += 90;
        }
}

Player.prototype.reset = function () {
    this.x = 200;
    this.y = 400;
};

// function to set score to zero

function zeroScore(){
    score = 0;
    theScore = `${scoreLabel} ${score}`;
    scoreDisplay.innerHTML = theScore;
    collisionSound.play();
    player.reset();
}

// function to add one from score 

function addOne() {
    score += 1;
    theScore = `${scoreLabel} ${score}`;
    scoreDisplay.innerHTML = theScore;
    player.reset();
}

// function to deduct one from score

function removeOne() {
    score -= 1;
    theScore = `${scoreLabel} ${score}`;
    scoreDisplay.innerHTML = theScore;
    collisionSound.play();
    player.reset();
}

muteSound.addEventListener("click", function(){
    this.classList.toggle("hide-mute");
    unMuteSound.classList.toggle("hide-mute");
    gameSound.sound.muted = true;
    collisionSound.sound.muted = true;
    playerSound.sound.muted = true;
    console.log(this);
    console.log(unMuteSound);
});

unMuteSound.addEventListener("click", function(){
    this.classList.toggle("hide-mute");
    muteSound.classList.toggle("hide-mute");
    gameSound.sound.muted = false;
    collisionSound.sound.muted = false;
    playerSound.sound.muted = false;
    console.log(this);
});

// Now write your own player class
// This class requires an update(), render() and
// a handleInput() method.


// Now instantiate your objects.
// Place all enemy objects in an array called allEnemies
// Place the player object in a variable called player
// Passing all objects into their respective variables

const enemyObject1 = new Enemy(-90, 60);
const enemyObject2 = new Enemy(-300, 140);
const enemyObject3 = new Enemy(-400, 220);
const enemyObject4 = new Enemy(-590, 140);
const enemyObject5 = new Enemy(-700, 220);
const enemyObject6 = new Enemy(-890, 60);
const allEnemies = [enemyObject1, enemyObject2, enemyObject3, enemyObject4, enemyObject5, enemyObject6];
const player = new Player(200, 400);
const gameSound = new Sound("sound/game-sound.mp3");
const playerSound = new Sound("sound/player-sound.mp3");
const collisionSound = new Sound("sound/collision-sound.mp3");


// This listens for key presses and sends the keys to your
// Player.handleInput() method. You don't need to modify this.
document.addEventListener('keyup', function(e) {
    var allowedKeys = {
        37: 'left',
        38: 'up',
        39: 'right',
        40: 'down'
    };

    player.handleInput(allowedKeys[e.keyCode]);
    playerSound.play();
});
